...
class DSpike(SurrogateFunctionBase):
    ...
    @staticmethod
    @script
    def primitive_function(x: Tensor, alpha: float):
        return (alpha*(x-0.5)).tanh_() / (2.*tanh(alpha/2.))+0.5


def getSpikingNode():
    return LIFNode(tau=5., decay_input=False, v_threshold=0.5, detach_reset=True, surrogate_function=DSpike())
    # return LIFNode(tau=5., decay_input=False, v_threshold=0.5, detach_reset=True)


class SpikingConv2d(nn.Module):
  def __init__(self, input_c, output_c, kernel_size=3, stride=1, padding=1, b=3, spiking=True):
  ...
  def forward(self,x):
    if self.training:
      ...
      x = fuse_conv_bn_eval(self.conv,self.bn)(x)
    if self.spiking:
      x = self.spike(x)
    return x
...
class SpikingLinear(nn.Module):
  ...
  def forward(self,x):
    if self.training:
      ...
      x = fuse_linear_bn_eval(self.linear,self.bn[0])(x)
    return self.spike(x)
...
#   def forward(self,x):
#     if x.shape[-1] == self._shape[-1] and x.shape[-2] == self._shape[-2]:
#       return x
#     return interpolate(x,self._shape,mode = "nearest")
